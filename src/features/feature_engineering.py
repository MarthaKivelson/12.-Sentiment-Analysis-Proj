...
def apply_bow(train_data: pd.DataFrame, test_data: pd.DataFrame, n_grams: int, max_features: int) -> tuple:
    """Apply Count Vectorizer to the data."""
    try:
        logging.info("Applying BOW...")
        vectorizer = TfidfVectorizer(ngram_range=n_grams, sublinear_tf=False, max_features=max_features)
...
def main():
    try:
        ...
        n_gram = params['feature_engineering']['n_gram']
        max_feature = params['feature_engineering']['max_features']
        # n_gram = (1, 3)

        train_data = load_data('./data/interim/train_processed.csv')
        test_data = load_data('./data/interim/test_processed.csv')
        train_df, test_df = apply_bow(train_data, test_data, tuple(n_gram), max_feature)
